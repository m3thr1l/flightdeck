...
keys Escape m; sleep 1; check "Alt-m cycles to files" "$(T list-panes -F '#{pane_title}' | tr '\n' ' ')" ' files'
keys Escape m; sleep 1; check "Alt-m cycles back" "$(T list-panes -F '#{pane_title}' | tr '\n' ' ')" ' man'
